...
namespace mjlib {
    ...
    ICamera::Message MVCamera::gainmessage()
    {
        ...
        if (state == MVST_SUCCESS) {
            Mess.ipAddr = std::to_string(CamInfo->mIpAddr[0]) + '.' +
                std::to_string(CamInfo->mIpAddr[1]) + '.' +
                std::to_string(CamInfo->mIpAddr[2]) + '.' +
                std::to_string(CamInfo->mIpAddr[3]);
            Mess.ModelName = CamInfo->mModelName;
            for (int var = 0; var < 6; ++var) {
                Mess.MAC.append(':' + std::to_string(uint(CamInfo->m_IfMAC[var])));
            }
