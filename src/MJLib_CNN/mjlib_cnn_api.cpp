 ...
 * @return 返回模型指针
*/
extern "C"  uint32_t * CNN_CreateModel()
{
 ...
 * @return 成功返回1，失败返回0
*/
extern "C"   int CNN_LoadModel(uint32_t* model, const char* path)
{
 ...
 * @return 返回空化概率
*/
extern "C"  float CNN_ReasoningModel(uint32_t * model, uint32_t * image)
{
	try
	{
		auto net = reinterpret_cast<mjlib::NeuralNetwork*>(model);
 ...
 * @return 成功返回1，失败返回0
*/
extern "C"   int CNN_DeleteModel(uint32_t * model)
{
