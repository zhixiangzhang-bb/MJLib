...
extern "C" {
	 ...
	 * @return 返回MAT图片指针
	*/
	uint32_t * LVImagePointer2CVImage(unsigned char* LVImagePointer, int lineWidth, int height, int width, int channels)
	{
		cv::Mat image;
		if (channels == 1) {
	 ...
	 * @return 返回MAT图片指针
	*/
	uint32_t * ImageRIO(uint32_t * image, int xStrt, int yStart, int xEnd, int yEnd)
	{
	 ...
	 * @return 返回MAT图片指针
	*/
	uint32_t * ImageGray(uint32_t * image)
	{
		cv::Mat src = *reinterpret_cast <cv::Mat*>(image);
