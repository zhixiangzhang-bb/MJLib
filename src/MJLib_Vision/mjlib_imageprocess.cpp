...
namespace mjlib {
	...
	******************************************************************/
	...
	ImageROI::ImageROI():xStrt(-1), yStart(-1), xEnd(-1), yEnd(-1)
	{
	...
	ImageROI::ImageROI(int xStrt, int yStart, int xEnd, int yEnd)
	{
		this->xStrt = xStrt;
		this->yStart = yStart;
		this->xEnd = xEnd;
	...
	cv::Mat ImageROI::processImage(const cv::Mat& image)
	{
		
		cv::Rect roi(xStrt, yStart, xEnd, yEnd);

		roi = roi & cv::Rect(0, 0, image.cols, image.rows);//判断图像是否超出边界
		return image(roi);;
	}
	...
	//设置参数
	void ImageROI::SetConfig(int xStrt, int yStart, int xEnd, int yEnd)
	{
		this->xStrt = xStrt;
		this->yStart = yStart;
		this->xEnd = xEnd;
	...
	//返回处理名称
	std::string ImageROI::ReturnName()
	{
		return "截取图像";
	}
